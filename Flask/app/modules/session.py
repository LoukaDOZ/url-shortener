...
class SessionManager():
    ...
    def get_session(self, session: fsession) -> Session:
        has_sid = self.__session_has_sid__(session)
        s = self.__find_session__(session["sid"]) if has_sid else None

        if not s or s.get("expiration_date") < int(time.time()):
            s = self.__create_session__(session, int(time.time()) + self.SESSION_LIFETIME)
        
        return s
    
    def __create_session__(self, session: fsession, expiration_date: int) -> None:
    ...
    def __session_has_sid__(self, session: fsession) -> bool:
        return "sid" in session and bool(session["sid"])
